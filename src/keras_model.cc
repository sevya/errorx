...
vector<double> keras::read_1d_array( ifstream &fin, int cols ) {
	vector<double> arr;
	double tmp_double;
	char tmp_char;
	fin >> tmp_char; // for '['
...
void keras::DataChunk2D::read_from_file( const string &fname ) {
...
void keras::LayerConv2D::load_weights(ifstream &fin) {
	char tmp_char = ' ';
	string tmp_str = "";
	double tmp_double;
	bool skip = false;
...
void keras::LayerConv2D::load_weights(istringstream &fin) {
	char tmp_char = ' ';
	string tmp_str = "";
	double tmp_double;
	bool skip = false;
...
void keras::LayerActivation::load_weights(ifstream &fin) {
	fin >> m_activation_type;
	//cout << "Activation type " << m_activation_type << endl;
}


void keras::LayerActivation::load_weights(istringstream &fin) {
	fin >> m_activation_type;
	//cout << "Activation type " << m_activation_type << endl;
}

void keras::LayerMaxPooling::load_weights(ifstream &fin) {
	fin >> m_pool_x >> m_pool_y;
	//cout << "MaxPooling " << m_pool_x << "x" << m_pool_y << endl;
}

void keras::LayerMaxPooling::load_weights(istringstream &fin) {
	fin >> m_pool_x >> m_pool_y;
	//cout << "MaxPooling " << m_pool_x << "x" << m_pool_y << endl;
}

void keras::LayerDense::load_weights(ifstream &fin) {
...
void keras::LayerDense::load_weights(istringstream &fin) {
	fin >> m_input_cnt >> m_neurons;
	double tmp_double;
	char tmp_char = ' ';
	for ( int i = 0; i < m_input_cnt; ++i ) {
...
keras::KerasModel::KerasModel( const string & input_fname, bool from_string, int verbose)
	...
	if ( from_string ) load_weights_from_string(input_fname);
	else load_weights(input_fname);
}

keras::KerasModel::KerasModel( const string &input_fname, int verbose)
		 : m_verbose(verbose) {
...
keras::DataChunk* keras::LayerFlatten::compute_output(keras::DataChunk* dc) {
	...
	int csize = im[0].size();
	int rsize = im[0][0].size();
	int size = im.size() * csize * rsize;
	keras::DataChunkFlat *out = new DataChunkFlat(size);
	double * y_ret = out->get_1d_rw().data();
	for ( int i = 0, dst = 0; i < im.size(); ++i ) {
...
vector<vector<double>> keras::conv_single_depth_valid(
	vector<vector<double>> const & im,
	vector<vector<double>> const & k)
{
	int k1_size = k.size(), k2_size = k[0].size();
	unsigned int st_x = (k1_size - 1) >> 1;
	unsigned int st_y = (k2_size - 1) >> 1;

	vector<vector<double>> y(im.size() - 2*st_x, vector<double>(im[0].size() - 2*st_y, 0));
...
vector<vector<double>> keras::conv_single_depth_same(
	vector<vector<double>> const & im,
	vector<vector<double>> const & k)
{
	int k1_size = k.size(), k2_size = k[0].size();
	int st_x = (k1_size - 1) >> 1;
	int st_y = (k2_size - 1) >> 1;

	int max_imc = im.size() - 1;
	int max_imr = im[0].size() - 1;
	vector< vector<double> > y(im.size(), vector<double>(im[0].size(), 0));
...
keras::DataChunk* keras::LayerConv2D::compute_output(keras::DataChunk* dc) {
	...
	int size_x = (m_border_mode == "valid")? im[0].size() - 2 * st_x : im[0].size();
	int size_y = (m_border_mode == "valid")? im[0][0].size() - 2 * st_y: im[0][0].size();
...
vector<double> keras::KerasModel::compute_output(keras::DataChunk *dc) const {
	//cout << endl << "KerasModel compute output" << endl;
	//cout << "Input data size:" << endl;
	if ( m_verbose > 1 ) dc->show_name();
	...
	if ( m_verbose > 1 ) out->show_values();
	delete out;

	return flat_out;
}

void keras::KerasModel::load_weights(const string &input_string) {
	if ( m_verbose > 1 ) cout << "Reading model from " << input_string << endl;
	...
	if ( m_verbose > 1 ) cout << "Layers " << m_layers_cnt << endl;

	for ( int layer = 0; layer < m_layers_cnt; ++layer ) { // iterate over layers
		fin >> tmp_str >> tmp_int >> layer_type;
		if ( m_verbose > 1 ) cout << "Layer " << tmp_int << " " << layer_type << endl;
		...
		if ( layer_type == "Convolution2D" ) {
			...
			continue; // we dont need dropout layer in prediction mode
		} else if ( layer_type == "BatchNormalization" ) {
			cout << "Error: BatchNormalization layer not supported. Exiting gracefully..." << endl;
			exit(0);
		} else {
...
void keras::KerasModel::load_weights_from_string( const string &input_string ) {
...
void keras::LayerDense::print_weights() {
	for ( int ii = 0; ii < m_weights.size(); ++ii ) {
		for ( int jj = 0; jj < m_weights[ii].size(); ++jj ) {
			cout << ii << "," << jj << " : " << m_weights[ii][jj] << endl;
		}
	}
}
